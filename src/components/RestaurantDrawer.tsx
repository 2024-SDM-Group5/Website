	...
	useEffect(() => {
		const FetchRestaurant = async () => {
			...
			const res = await axios.get(
				`https://mainserver-service:8080/api/v1/restaurants/${show}`,
				{
			...
			for (let i = 0; i < res.data.diaries.length; i++) {
				const tmp = await axios.get(
					`https://mainserver-service:8080/api/v1/diaries/${res.data.diaries[i].id}`,
				);
				let d = new Date();
						...
						<div className="flex w-[105px] flex-row justify-center pt-[15px]">
								...
								<div
									...
									onClick={async (e) => {
										const res = await axios.delete(
											`https://mainserver-service:8080/api/v1/restaurants/${restaurant.placeId}/like`,
											{
												headers: {
							...
							{restaurant.hasDisliked ? (
								<div
									...
									onClick={async (e) => {
										const res = await axios.delete(
											`https://mainserver-service:8080/api/v1/restaurants/${restaurant.placeId}/dislike`,
											{
												headers: {
													Authorization: `Bearer ${session.data?.idToken}`,
												},
						...
						<div style={{ display: 'inline' }}>
							{restaurant?.hasCollected ? (
								<Button
									...
									onClick={async (e) => {
										const res = await axios.delete(
											`https://mainserver-service:8080/api/v1/restaurants/${restaurant.placeId}/collect`,
											{
												headers: {
													Authorization: `Bearer ${session.data?.idToken}`,
												},
